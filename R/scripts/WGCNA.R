...
# Load data
datExpr <- read.csv("~/Bioinformatics/R/data/trt.vs.ctr_gexp.mat.csv",
                    row.names=1)
datExpr <- t(datExpr)   # format for WGCNA tool
datExpr <- datExpr[,1:500]   # subset for quicker calculations
datExpr[,c(1:100,300:400)] <- datExpr[,c(1:100,300:400)]/3.24   # format dummy dataset
datExpr[,c(101:200,401:500)] <- datExpr[,c(101:200,401:500)]/3.24   # format dummy dataset

datTraits <- read.csv("~/Bioinformatics/R/data/trt.vs.ctr_metadata.csv",
                      row.names=1)
datTraits <- datTraits[,2:3]  # subset to numeric traits
